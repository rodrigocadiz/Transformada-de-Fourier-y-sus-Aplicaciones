...
!!! alert "Definición"
	La óptica es la rama de la física que estudia el comportamiento y las propiedades de la luz, incluidas sus interacciones con la materia, así como la construcción de instrumentos que se sirven de ella o la detectan. 
!!! note "Sistemas Ópticos"
	Los sistemas ópticos son cualquier combinación de objetos que separan medios con distinto índice de refracción que cambian el camino de la luz para formar imagenes. **Los sistemas ópticos son sistemas lineales e invariantes que están caracterizados por su respuesta al impulso**.
...
md"""
!!! alert "Definición"
	En este análisis físico se considera que la luz es una onda que se refleja y se refracta. **La óptica geométrica es válida cuando la longitud de onda de la luz que atraviesa el sistema óptico es de mucho menor tamaño que los elementos que componen sistema**, ya que modela el comportamiento de la luz como un rayo.
"""
...
!!! note "Fórmula de Rayleigh-Sommerfeld"
	...
	Considerando que la distribución de campo incidente $u_{i n}(x, y)$ se distribuye uniformemente en el área de la apertura (a):
...
!!! alert "Aproximación de Fraunhoffer"
	Cuando la fuente de campo es lejana al sistema óptico, es posible decir que $\boldsymbol{R}>>d$ donde $d$ es el diámetro de la apertura y $R$ es la distancia del centro de la apertura al punto. De esta forma:
	...
	Podemos observar que **en aproximación de campo lejano, el valor del campo eléctrico en un punto ($\mathbf{x'}$,$\mathbf{y'}$) corresponde a la Transformada de Fourier 2D aplicada a la función apertura evaluada en las variables de frecuencia $\left(\mathbf{\frac{kx'}{R}},\mathbf{\frac{ky'}{R}}\right)$**. Por simplicidad en el futuro realizaremos el siguiente cambio de variable:

	$$u=\frac{kx'}{R} \qquad v=\frac{ky'}{R}$$

	Por lo tanto, el campo eléctrico en un punto $\mathbf{x}^{\prime}$ a una distancia $R$ del sistema óptico es:

	$u\left(\mathbf{x}^{\prime}\right) = \mathcal{F}\left\{A(a,y)\right\}\left( u,v\right)$
"""
...
!!! note "PSF"
	La salida de un sistema óptico ($g(x,y)$) corresponde a la convolución entre la entrada ($f(x,y)$) y su PSF (respuesta al impulso):

	$$g(x,y) = \left\{f*PSF\right\}(x,y)$$

	La PSF de un sistema óptico es el módulo cuadrado de la transformada de Fourier de la apertura:
...
begin
	...
	plot!(pim1, pconv2, pconv1, layout=(1,3)) 
end
